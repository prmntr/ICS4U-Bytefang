...
public class ComputerPlayer extends Players {
    ...
    public boolean makeGuess() {
        // once there is one charecter left on the board, than the computer is ready to
        // make a guess
        // The computer will only guess when it knows for certain what charecter is the
        // HumanPlayers charecter
        return (getBoard().getCharacterList().size() == 1);
    }

    @Override
    // quesiton is returned in computerDecisionMaking, so there is no need for the
    // askQuestion() method for ComputerPlayer
    ...
    public String computerDecisionMaking() {
        ...
        // assume that the safest question and the most riskiest question are the first
        // question in the charecter list (for now)
        ...
        for (String question : questionList) {
            ...
            // situation in which current question's yes/no is closer to 50/50 than other
            // questions
            ...
            // sutation in which the current questions's yes/no split is more unbalanced
            // (riskest question)
